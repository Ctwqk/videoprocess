...
**Architecture:** PostgreSQL stores service-scoped token grants and epoch-fenced 45-second registrations. Python and Go workers register directly before their first Redis command, renew every 15 seconds, and cancel on lease loss; the deploy controller provisions Docker secrets/grants and the five-minute watcher applies a global fail-closed hold for unknown consumers.
...
- Lease duration is exactly 45 seconds; heartbeat interval is exactly 15 seconds.
- Raw admission tokens never enter environment variables, command arguments, logs, evidence, or PostgreSQL.
- Every grant is scoped to one service, worker type, host, and exact sorted capability set.
...
validation, token hashing, no raw secret persistence, 45-second expiry,
15-second renewal semantics, concurrent takeover, stale epoch heartbeat,
expiry, revoke, and cross-language canonical fingerprint fixtures.
...
duplicate, or unsupported values. On register, revoke the previous active row,
advance `max(lease_epoch)+1`, and insert the replacement in one transaction.
Heartbeat updates only the exact active registration/instance/epoch before
expiry. Revoke is idempotent.
...
message tasks and leave their deliveries pending. Revoke with a fresh bounded
context in `finally`.
...
Cover token/claim checks, 45-second lease, 15-second heartbeat, concurrent
...
no acknowledgement after loss, and sanitized errors.
...
loss cancels the consumer context and prevents completion acknowledgement.
...
Require mode `0600` state, 256-bit tokens, one secret per service, no token in
commands/logs/env/database output, exact service/type/host/capabilities,
migration before grant and worker update, exact image identity injection,
registration readiness before proceeding, rollback on readiness failure,
...
Run backend migration and verify head before updating workers. Create state
atomically with `umask 077`; create a service-scoped Swarm secret from stdin
without echoing it. Mount only that service's secret with mode `0400` and inject
`WORKER_SERVICE_NAME`, `WORKER_IMAGE_IDENTITY`, `WORKER_CAPABILITIES`, and
`WORKER_ADMISSION_TOKEN_FILE`. Upsert the hashed grant through the Python worker
image. After service convergence, query registration readiness through a
sanitized CLI and enter the existing image rollback path on failure.
...
- Managed watcher interval becomes exactly five minutes.
...
or Redis uncertainty; sanitized output; five-minute cron; and no mutation when
...
evidence file permissions, no secret output, and strict source-level zero-write
behavior.
