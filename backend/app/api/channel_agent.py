...
async def read_owned_inventory(channel_id: uuid.UUID, inventory_id: uuid.UUID, db: AsyncSession = Depends(get_db)):
    return await _inventory_result(db, owned_inventory.read_inventory(db, channel_id, inventory_id))
...
async def task_audit(task_id: str, db: AsyncSession = Depends(get_db)):
    ...
    decision = (
        await db.execute(
            ...
            .order_by(DecisionAuditEntry.selected.desc(), DecisionAuditEntry.created_at.desc(), DecisionAuditEntry.id.desc())
            .limit(1)
        )
    ).scalars().first()
    publication = (
    ...
    if publication is not None:
        material_rows = (
            await db.execute(
                ...
                .order_by(MaterialUsageLedger.used_at.desc(), MaterialUsageLedger.id.desc())
                .limit(200)
            )
        ).scalars().all()
    return {
