...
def main() -> int:
    parser = argparse.ArgumentParser(description="Create an AutoFlow hot-topic explainer demo plan.")
    ...
    _require_plan(plan, expected_intent="hot_topic_explainer", expected_template="hot_topic_explainer_short")
    _require_rights(plan, expected_status="review_required", expected_review=True)
...
def _summary(plan: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "rights_reasons": rights.get("reasons", []),
        "needs_review": plan.get("needs_review"),
        "candidate_count": len(plan.get("candidates", [])),
        "candidate_rights": sorted({candidate.get("rights_status") for candidate in plan.get("candidates", [])}),
