...
def test_upgrade_does_not_restore_legacy_marker_delegation(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    events: list[str] = []
    ...
    async def connect(*_args, **_kwargs) -> None:
        pytest.fail("schema migration must not restore legacy role delegation")
...
def test_current_deployment_applies_owned_producer_fence_042():
    ...
    assert worker_deployment_cli.EXPECTED_MIGRATION_HEAD == "045_registered_consumer_history"
